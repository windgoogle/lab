...
// Set nonblocking mode for the listening socket
        serverChannel.configureBlocking(false);     // false 否则register时java.nio.channels.IllegalBlockingModeException
...
// Loop while data is available; channel is nonblocking
        while ((count = socketChannel.read(buffer)) > 0) {
            buffer.flip(); // Make buffer readable    //切换到读模式 by huangfeng
// Send the data; don't assume it goes all at once
            while (buffer.hasRemaining()) {
                socketChannel.write(buffer);   //写回到 通道中 ,by huangfeng
            }
            //added by huangfeng
           // buffer.compact();
            //buffer.put("\r\n".getBytes());
